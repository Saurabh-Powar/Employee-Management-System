...
const markAsRead = async (req, res) => {
  ...
  try {
    const result = await pool.query("UPDATE notifications SET is_read = true WHERE id = $1 RETURNING *", [
      notificationId,
    ])
...
const markAllAsRead = async (req, res) => {
  ...
  try {
    await pool.query("UPDATE notifications SET is_read = true WHERE employee_id = $1", [employeeId])
...
const getUnreadCount = async (req, res) => {
  ...
  try {
    const result = await pool.query("SELECT COUNT(*) FROM notifications WHERE employee_id = $1 AND is_read = false", [
      employeeId,
    ])
