...
const getEmployeeTasks = async (req, res) => {
  ...
  try {
    const result = await pool.query("SELECT * FROM tasks WHERE assigned_to = $1 ORDER BY due_date ASC", [employeeId])
...
const updateTaskStatus = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query("UPDATE tasks SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING *", [
      dbStatus,
      taskId,
    ])
