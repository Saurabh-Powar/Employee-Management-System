        ...
        {tasks.map((task) => (
          <div key={task.id} className={`task-item ${isOverdue(task.due_date, task.status) ? "overdue" : ""}`}>
            <div className="task-header" onClick={() => toggleTaskDetails(task.id)}>
              ...
              <div className={`task-status ${getStatusClass(task.status)}`}>{task.status.replace("_", " ")}</div>
            </div>

            <div className="task-meta">
              <div className={`task-priority ${getPriorityClass(task.priority)}`}>{task.priority || "Normal"}</div>
              <div className="task-due-date">Due: {formatDate(task.due_date)}</div>
            </div>
              ...
              <div className="task-details">
                ...
                {(user.role === "employee" || !employeeId) && (
                  <div className="task-actions">
                    {task.status !== "pending" && (
                      <button className="btn-status pending" onClick={() => handleStatusChange(task.id, "pending")}>
                        Mark as Pending
                      </button>
                    )}
                    ...
                    {task.status !== "completed" && (
                      <button className="btn-status completed" onClick={() => handleStatusChange(task.id, "completed")}>
                        Mark as Completed
                      </button>
                    )}
                  </div>
